...
from traits.api import (Bool, Event, Float, HasStrictTraits, Instance, List,
...
class ApplicationEvent(HasStrictTraits):
    ...
    application = Instance('TaskGuiApplication')

    #: The type of application event.
    event_type = Str


class TaskGuiApplication(HasStrictTraits):
    ...
    splash_screen_duration = Float(3)

    #: Icon for the application
    icon = Instance(ImageResource)

    #: Instance of an AboutDialog to show in the Help menu
    about_dialog = Instance(IAboutDialog)
    ...
    tasks_created = List(Instance("pyface.tasks.task.Task"))
    ...
    def create_task_window(self, task):
        ...
        window.on_trait_change(self._on_closing_window, 'closing')
    ...
    def _on_closing_window(self, window, trait, old, new):
        """ Catch the window closing request before it is processed for a
        chance to veto it. That is done by setting new.veto to True.
        """
